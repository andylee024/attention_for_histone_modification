...
def _handle_directory_creation(experiment_config):
    ...
    if os.path.isdir(experiment_config.experiment_directory):
        ...
        # hack because ensure_directory() does not work on lane
        if not os.path.isdir(experiment_config.experiment_directory):
            os.mkdir(experiment_config.experiment_directory)
        if not os.path.isdir(experiment_config.checkpoint_directory):
            os.mkdir(experiment_config.checkpoint_directory)
        if not os.path.isdir(experiment_config.config_directory):
            os.mkdir(experiment_config.config_directory)
        if not os.path.isdir(experiment_config.summary_directory):
            os.mkdir(experiment_config.summary_directory)
