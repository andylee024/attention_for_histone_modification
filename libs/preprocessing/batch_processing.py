...
def partition_and_annotate_data(
        sequences, labels, extractor, partition_size=1000):
...
def create_dataset_from_attention_partition(base_config, attention_partition):
    ...
    return AttentionDataset(
        config=_convert_to_partition_dataset_config(
            base_config, attention_config),
        training_examples=_generate_training_examples_from_attention_partition(attention_partition))


def _generate_training_examples_from_attention_partition(attention_partition):
...
def _convert_to_partition_dataset_config(base_config, attention_partition):
    ...
    updated_config.dataset_name = "{base_dataset_name}_{start}_{end}".format(base_dataset_name=config.dataset_name,
                                                                             start=partition_data.indices[0],
                                                                             end=partition_data.indices[-1])
    updated_config.indices = partition_data.indices
